...
template<typename V> class SArray {
 public:
  ...
  size_t activeMemSize() const { return size_*sizeof(V); }
  ...
  bool writeToFile(const string& file_name, const bool sync_to_device = false) const {
    return writeToFile(SizeR(0, size_), file_name, sync_to_device);
  }
  // write the segment [range.begin(), range.end()) into a binary file
  bool writeToFile(SizeR range, const string& file_name, const bool sync_to_device = false) const;
